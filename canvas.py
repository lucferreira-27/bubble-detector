...
with open('One Piece - c0001 (v001) - p011 [VIZ Media] [Digital] [1r0n].png_2023-05-12_02-32_results.json') as f:
...
class Word:
    def __init__(self, vertices):
        ...
        self.cx = (self.x1 + self.x2 + self.x3 + self.x4) / 4 # average x coordinate
        self.cy = (self.y1 + self.y2 + self.y3 + self.y4) / 4 # average y coordinate
...
words = [Word(text['vertices']) for text in texts[1:]] # skip the first element which is the whole text
...
def sort_blocks(blocks):
    ...
    while blocks:
        ...
        same_row = [block for block in blocks if block.min_y <= highest_block.max_y]
...
im = Image.open('One Piece - c0001 (v001) - p011 [VIZ Media] [Digital] [1r0n].png')
draw = ImageDraw.Draw(im)
font = ImageFont.truetype("arial.ttf", 32) # load a font for drawing numbers

for i, block in enumerate(blocks_list):
    draw.rectangle([block.min_x, block.min_y, block.max_x, block.max_y], outline='red')
    draw.text((block.cx, block.cy), str(i + 1), fill='red', font=font, anchor='mm') # draw the number in the middle of the block
